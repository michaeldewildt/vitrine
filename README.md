...
- The **`spawn-issued` in-flight guard**: a spawn is recorded the moment it is issued; a wait loop that attaches inside the wrapper's boot window (the state still `queued`) sees the fresh event and does not double-spawn — an event older than the stuck window is a dead spawner, and the loop re-spawns.
